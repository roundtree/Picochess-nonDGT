...
class EventApi():
    ...
    Lift_Piece = 'EVT_LIFT_PIECE'  # Added by BL
    Drop_Piece = 'EVT_DROP_PIECE'  # Added by BL
...
class MessageApi():
    ...
    LIGHT_SQUARE = 'MSG_LIGHT_SQUARE'  # BL   light up a square
    PLAYMOVE = 'MSG_PLAY_MOVE'          #bl instead of go command
...
class Message():
    ...
    ENGINE_STARTUP = ClassFactory(MessageApi.ENGINE_STARTUP, ['name', 'shell', 'file', 'has_levels', 'has_960'])
    ...
    USER_TAKE_BACK = ClassFactory(MessageApi.USER_TAKE_BACK, ['game'])
    ...
    LIGHT_SQUARE = ClassFactory(MessageApi.LIGHT_SQUARE, ['square', 'on'])
    PLAYMOVE = ClassFactory(MessageApi.PLAYMOVE, ['fen'])
...
class Event():
    ...
    DROP_PIECE = ClassFactory(EventApi.Drop_Piece, ['square'])
